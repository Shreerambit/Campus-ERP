...
export async function fetchStudents(opts: FetchStudentsOpts = {}): Promise<DBStudent[]> {
  const sb = ensureClient();
  let q = sb.from('v_students_public').select('*');
  ...
  let q2 = sb.from('students').select('*');
  if (opts.collegeId) q2 = q2.eq('college_id', opts.collegeId);
  if (opts.courseId)  q2 = q2.eq('course_id',  opts.courseId);
  if (opts.semester)  q2 = q2.eq('semester',   opts.semester);
  if (opts.section)   q2 = q2.eq('section',    opts.section);
  if (opts.search) {
    const s = opts.search.trim();
    q2 = q2.or(`reg_no.ilike.%${s}%,name.ilike.%${s}%`);
  }
  q2 = q2.order('roll_number').limit(opts.limit ?? 500);
  const { data: d2 } = await q2;
  return (d2 || []) as DBStudent[];
}
