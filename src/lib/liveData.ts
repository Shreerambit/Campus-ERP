...
import { lookupDepartment, lookupCourse, ensureDeptCourseLookups } from './tenant';
...
export function dbToStudent(db: DBStudent, index = 0): Student {
  // `db` may carry extra columns from v_students_public (course_name, department_name).
  // Fall back to tenant lookups before generic labels.
  const dbAny = db as any;

  const courseLookup = lookupCourse(db.course_id);
  const deptLookup = lookupDepartment(db.department_id)
    || (courseLookup?.department_id ? lookupDepartment(courseLookup.department_id) : undefined);

  const courseName     = dbAny.course_name     || courseLookup?.name || dbAny.course_code     || courseLookup?.code || 'Unknown Course';
  const departmentName = dbAny.department_name || deptLookup?.name   || dbAny.department_code || deptLookup?.code   || 'Unknown Department';
  const courseCode     = dbAny.course_code     || courseLookup?.code || '';
  const departmentCode = dbAny.department_code || deptLookup?.code   || '';

  // Section: only store A/B explicitly; empty string means not yet assigned.
  const section = db.section ? db.section.toUpperCase() : '';
  ...
  return {
    ...
    section,
    batch_no: '',
    // These are read from DB or dynamically resolved from tenant lookup.
    department: departmentName,
    course: courseName,
    course_code: courseCode,
    department_code: departmentCode,
    semester: ['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII'][db.semester - 1] || String(db.semester),
...
export function useCollegeStudents(collegeId?: string, filters: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Student[]> => {
      ...
      await ensureDeptCourseLookups(collegeId);
 ...
 */
/**
 * Students with real attendance %, optionally filtered by course + semester.
 * Used by Directory + Rankings — filters at the DB level, not in JS.
 */
export function useCollegeStudentsWithAttendance(
  collegeId?: string,
  filters: { courseId?: string; semester?: number; section?: string } = {}
) {
  return useQuery({
    queryKey: ['students-with-att', collegeId, filters],
    enabled: !!collegeId && HAS_SUPABASE,
    staleTime: 30_000,
    queryFn: async (): Promise<Student[]> => {
      ...
      // Build query with optional course/semester/section scoping
      let q = supabase.from('v_students_public').select('*').eq('college_id', collegeId);
      ...
      q = q.order('roll_number').limit(2000);

      const { data: pubRows, error: pubErr } = await q;

      if (!pubErr && pubRows && pubRows.length > 0) {
        rows = pubRows;
      } else {
        // Fallback to raw students table if view unavailable
        let q2 = supabase.from('students').select('*')
          .eq('college_id', collegeId).eq('status', 'active');
        if (filters.courseId)  q2 = q2.eq('course_id', filters.courseId);
        if (filters.semester)  q2 = q2.eq('semester',  filters.semester);
        if (filters.section)   q2 = q2.eq('section',   filters.section);
        q2 = q2.order('roll_number').limit(2000);
        const { data: stRows } = await q2;
        rows = stRows || [];
      }

      // Attendance view — scoped to college (and optionally course/semester)
      let attQ = supabase.from('v_student_attendance')
        .select('student_id, pct, present, total').eq('college_id', collegeId);
      if (filters.courseId)  attQ = attQ.eq('course_id', filters.courseId);
      if (filters.semester)  attQ = attQ.eq('semester',  filters.semester);
...
/**
 * Rankings for one student, scoped to their specific course + semester.
 * This ensures BCA Sem 5 ranks are among BCA Sem 5 students only.
 */
export function useStudentRanks(
  collegeId?: string,
  regNo?: string,
  section?: string,
  courseId?: string,
  ...
  return useQuery({
    queryKey: ['ranks', collegeId, regNo, section, courseId, semester],
    enabled: !!collegeId && !!regNo && HAS_SUPABASE,
    staleTime: 30_000,
    queryFn: async (): Promise<{ overall: Rank; sectionRank: Rank; attendance: Rank }> => {
      ...
      // Scope: same college + same course + same semester (peers only)
      let all: any[] = [];
      let q = supabase.from('v_students_public')
        .select('reg_no, cgpa, section')
        .eq('college_id', collegeId);
      if (courseId)  q = q.eq('course_id', courseId);
      if (semester)  q = q.eq('semester',  semester);
      const { data: pubData, error: pubErr } = await q;

      if (!pubErr && pubData && pubData.length > 0) {
        all = pubData;
      } else {
        let q2 = supabase.from('students')
          .select('reg_no, cgpa, section')
          .eq('college_id', collegeId).eq('status', 'active');
        if (courseId)  q2 = q2.eq('course_id', courseId);
        if (semester)  q2 = q2.eq('semester',  semester);
        const { data: stData } = await q2;
        all = stData || [];
      }

      // Attendance also scoped to same course+semester
      let attQ = supabase.from('v_student_attendance')
        .select('reg_no, pct').eq('college_id', collegeId);
      if (courseId)  attQ = attQ.eq('course_id', courseId);
      if (semester)  attQ = attQ.eq('semester',  semester);
      const { data: att } = await attQ;
      ...
      return {
        overall:     { rank: overallIdx >= 0 ? overallIdx + 1 : 0, total: arr.length },
        sectionRank: { rank: secIdx >= 0 ? secIdx + 1 : 0,         total: inSec.length },
        attendance:  { rank: attIdx >= 0 ? attIdx + 1 : 0,         total: attByReg.length }
      };
    }
  });
}
...
/** Detect the note kind from a File object. */
export function getFileKind(file: File): Note['kind'] {
  const mime = file.type.toLowerCase();
  const ext  = file.name.split('.').pop()?.toLowerCase() || '';
  if (mime === 'application/pdf' || ext === 'pdf')                          return 'pdf';
  if (mime.startsWith('image/'))                                             return 'pdf'; // stored as attachment; display as doc icon
  if (['ppt','pptx'].includes(ext) || mime.includes('presentat'))           return 'ppt';
  if (['doc','docx'].includes(ext) || mime.includes('word') || mime.includes('opendocument.text')) return 'doc';
  if (['xls','xlsx','csv'].includes(ext) || mime.includes('spreadsheet') || mime.includes('excel')) return 'doc';
  if (mime.startsWith('video/'))                                             return 'video';
  return 'doc'; // fallback
}

export async function uploadNoteFile(collegeId: string, subjectCode: string, file: File): Promise<string> {
  if (!supabase) throw new Error('Supabase is not configured.');
...
/** @deprecated Use uploadNoteFile instead */
export const uploadNotePdf = uploadNoteFile;
...
export type CollegeSummaryStat = {
  college_id: string;
  college_name?: string;
  college_code?: string;
  status?: string;
...
export function useSuperAdminCollegeStats() {
  return useQuery({
    queryKey: ['super:college-stats'],
    enabled: HAS_SUPABASE,
    staleTime: 30_000,
    queryFn: async (): Promise<Record<string, CollegeSummaryStat>> => {
      if (!supabase) return {};
      // 1. Try view first
      const { data: viewData, error: viewErr } = await supabase.from('v_college_summary').select('*');
      if (!viewErr && viewData && viewData.length > 0) {
        const map: Record<string, CollegeSummaryStat> = {};
      ...
      // 2. Direct fallback aggregation
      const [{ data: st }, { data: tc }] = await Promise.all([
        supabase.from('students').select('college_id').eq('status', 'active'),
        supabase.from('teachers').select('college_id')
      ]);

      const map: Record<string, CollegeSummaryStat> = {};
      for (const s of st || []) {
        if (!map[s.college_id]) {
          map[s.college_id] = { college_id: s.college_id, student_count: 0, teacher_count: 0, department_count: 0, course_count: 0 };
        }
        map[s.college_id].student_count++;
      }
      for (const t of tc || []) {
        if (!map[t.college_id]) {
          map[t.college_id] = { college_id: t.college_id, student_count: 0, teacher_count: 0, department_count: 0, course_count: 0 };
        }
        map[t.college_id].teacher_count++;
      }
      return map;
    }
  });
...
export type AdminDashboardStats = {
  ...
  courseBreakdown: { courseId: string; courseCode: string; semester: number; count: number }[];
};

export function useAdminDashboardStats(collegeId?: string) {
  return useQuery({
    queryKey: ['admin:dashboard-stats', collegeId],
    enabled: !!collegeId && HAS_SUPABASE,
    staleTime: 30_000,
    queryFn: async (): Promise<AdminDashboardStats> => {
      ...
      if (!collegeId || !supabase) return empty;
      ...
      try {
        const [
          ...
          supabase.from('students').select('id, course_id, semester').eq('college_id', collegeId).eq('status', 'active'),
          supabase.from('teachers').select('id').eq('college_id', collegeId),
          supabase.from('departments').select('id').eq('college_id', collegeId),
          supabase.from('courses').select('id, code, name'),
          supabase.from('notices').select('id').eq('college_id', collegeId),
          supabase.from('leave_applications').select('id').eq('college_id', collegeId).eq('status', 'pending')
        ]);
        ...
        // Group students by course_id + semester
        const breakdownMap = new Map<string, { courseId: string; courseCode: string; semester: number; count: number }>();
        for (const s of stRows || []) {
          const key = `${s.course_id}:${s.semester}`;
          const cur = breakdownMap.get(key) || {
            courseId: s.course_id,
            courseCode: courseCodeMap.get(s.course_id) || 'Course',
            semester: s.semester,
            count: 0
          };
          cur.count++;
        ...
        return {
          studentCount: (stRows || []).length,
          teacherCount: (tcRows || []).length,
          courseCount: (courseRows || []).length,
          departmentCount: (deptRows || []).length,
          noticeCount: (notRows || []).length,
          pendingLeaves: (leaveRows || []).length,
          courseBreakdown: Array.from(breakdownMap.values()).sort((a, b) => a.courseCode.localeCompare(b.courseCode) || a.semester - b.semester)
        };
      } catch (e) {
