...
export function dbToStudent(db: DBStudent, index = 0): Student {
  ...
  // Fall back to generic labels only if the view doesn't provide them.
  const dbAny = db as any;
  const courseName     = dbAny.course_name     || dbAny.course_code     || 'Unknown Course';
  const departmentName = dbAny.department_name || dbAny.department_code || 'Unknown Department';
  ...
  return {
    ...
    section,
    batch_no: '',
    // These are now read from DB — never hardcoded.
