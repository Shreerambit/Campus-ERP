...
export function loadCachedTenant(): College[] {
  try {
...
/* ---------- Department & Course fast lookups ---------- */
export type DeptLookup = { id: string; code: string; name: string; college_id?: string };
export type CourseLookup = { id: string; code: string; name: string; department_id?: string };

const deptLookupMap = new Map<string, DeptLookup>();
const courseLookupMap = new Map<string, CourseLookup>();

export function primeLookupsFromColleges(cols: College[]) {
  for (const c of cols) {
    for (const d of c.departments || []) {
      if (d.id) {
        const item: DeptLookup = { id: d.id, code: d.code, name: d.name, college_id: c.id };
        deptLookupMap.set(d.id, item);
        deptLookupMap.set(d.id.toLowerCase(), item);
        if (d.code) deptLookupMap.set(d.code.toLowerCase(), item);
      }
      for (const cs of d.courses || []) {
        if (cs.id) {
          const item: CourseLookup = { id: cs.id, code: cs.code, name: cs.name, department_id: d.id };
          courseLookupMap.set(cs.id, item);
          courseLookupMap.set(cs.id.toLowerCase(), item);
          if (cs.code) courseLookupMap.set(cs.code.toLowerCase(), item);
        }
      }
    }
  }
...
// Prime immediately from cache on module load
const initialCached = loadCachedTenant();
if (initialCached.length > 0) {
  primeLookupsFromColleges(initialCached);
}

export function lookupDepartment(deptId?: string): DeptLookup | undefined {
  if (!deptId) return undefined;
  return deptLookupMap.get(deptId) || deptLookupMap.get(deptId.toLowerCase());
}

export function lookupCourse(courseId?: string): CourseLookup | undefined {
  if (!courseId) return undefined;
  return courseLookupMap.get(courseId) || courseLookupMap.get(courseId.toLowerCase());
}

let lookupsPromise: Promise<void> | null = null;
export async function ensureDeptCourseLookups(_collegeId?: string): Promise<void> {
  if (deptLookupMap.size > 0 && courseLookupMap.size > 0) return;
  if (lookupsPromise) return lookupsPromise;
  if (!HAS_SUPABASE || !supabase) return;

  lookupsPromise = (async () => {
    try {
      const [{ data: deps }, { data: crs }] = await Promise.all([
        supabase.from('departments').select('id, code, name, college_id'),
        supabase.from('courses').select('id, code, name, department_id')
      ]);
      for (const d of deps || []) {
        if (d.id) {
          const item: DeptLookup = { id: d.id, code: d.code, name: d.name, college_id: d.college_id };
          deptLookupMap.set(d.id, item);
          deptLookupMap.set(d.id.toLowerCase(), item);
          if (d.code) deptLookupMap.set(d.code.toLowerCase(), item);
        }
      ...
      for (const c of crs || []) {
        if (c.id) {
          const item: CourseLookup = { id: c.id, code: c.code, name: c.name, department_id: c.department_id };
          courseLookupMap.set(c.id, item);
          courseLookupMap.set(c.id.toLowerCase(), item);
          if (c.code) courseLookupMap.set(c.code.toLowerCase(), item);
        }
      }
    } catch (e) {
      console.warn('[tenant] ensureDeptCourseLookups failed', e);
    } finally {
      lookupsPromise = null;
    }
  })();
...
export function TenantProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setColleges(built);
      primeLookupsFromColleges(built);
