...
import { useSuperAdminCollegeStats } from '../lib/liveData';
...
export default function SuperAdmin() {
  ...
  const { data: statsMap = {} } = useSuperAdminCollegeStats();
  const totalStudents = Object.values(statsMap).reduce((acc, s) => acc + s.student_count, 0);
    ...
    <div className="min-h-dvh p-4 md:p-8 safe-bottom">
      ...
      <section className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {colleges.map(c => {
            const count = statsMap[c.id]?.student_count ?? 0;
            return (
