...
export default function Notes() {
  ...
  const { data: notes = [], isLoading, isError, error, refetch, isFetching } =
    useNotes(collegeId, { semester: targetSemester });
            ...
            <div className="flex items-start gap-3 min-w-0 mb-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] opacity-60 clip-1">
                  {n.subject?.code ? <>{n.subject.code} · </> : null}
                  {n.teacher?.name || 'Faculty'} · {new Date(n.created_at).toLocaleDateString()}
                </div>
              </div>
              {n.subject?.code && <span className="chip shrink-0">{n.subject.code}</span>}
            </div>
...
function ComposeSheet({ targetSemester, onClose }: { targetSemester: number; onClose: () => void }) {
  ...
  // Teacher/admin can pick ANY subject for this semester (same as Attendance).
  // We deliberately do NOT filter by assigned_subjects — subject-code drift
  // and missing assignments were locking teachers out of their own classes.
  const semSubjects = useMemo(
    () => allSubjects.filter(s => s.semester === targetSemester),
    [allSubjects, targetSemester]
  );
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (file) {
        const code = semSubjects.find(s => s.id === subjectId)?.code || 'general';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      >
        ...
        <select value={subjectId} onChange={e => setSubjectId(e.target.value)}
          ...
          {semSubjects.length === 0 && <option value="">(No subjects defined for this semester — ask admin)</option>}
          {semSubjects.map(s => <option key={s.id} value={s.id}>{s.code} · {s.name}</option>)}
        </select>
