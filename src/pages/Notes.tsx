...
import {
  ...
  Image, FileSpreadsheet, MonitorPlay, Video, File
} from 'lucide-react';
          ...
          <motion.article key={n.id}
            ...
            <div className="flex items-start gap-3 min-w-0 mb-2">
              <div className="h-10 w-10 rounded-xl grid place-items-center text-white shrink-0 bg-gradient-to-br from-ios-blue to-ios-indigo">
                {n.path_or_url ? <NoteKindIcon kind={n.kind} size={16}/> : <BookOpen size={16}/>}
              </div>
            ...
            {n.path_or_url && (
              <div className="mt-3 rounded-2xl border border-white/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-3 flex items-center gap-3">
                <div className={`h-9 w-9 rounded-xl text-white grid place-items-center shrink-0 ${noteKindGradient(n.kind)}`}>
                  <NoteKindIcon kind={n.kind} size={16}/>
                </div>
                <div className="text-xs opacity-70 flex-1 min-w-0 clip-1">
                  {n.path_or_url.split('/').pop()?.replace(/^\d+-/, '') || 'Attachment'}
                  <span className="ml-1 opacity-60 uppercase">[{noteKindLabel(n.kind)}]</span>
                </div>
...
function ComposeSheet({ targetSemester, onClose }: { targetSemester: number; onClose: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!body.trim() && !file) return setError('Add some text or attach a file.');

    setUploading(true);
    try {
      let path_or_url: string | null = null;
      let fileKind: ReturnType<typeof getFileKind> = null;
      if (file) {
        const code = semSubjects.find(s => s.id === subjectId)?.shortCode || 'general';
        path_or_url = await uploadNoteFile(user.college_id, code, file);
      ...
      await dispatchErpNotification({
        ...
        message: `${user?.displayName || 'Faculty'} shared study materials: "${title.trim()}".`,
        link: '/notes'
      });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      >
        ...
        <label className="text-[11px] uppercase tracking-wider font-semibold opacity-70">File attachment (optional)</label>
        <label className="mt-1 mb-3 w-full rounded-2xl border border-dashed border-white/60 dark:border-white/10 bg-white/60 dark:bg-white/5 px-4 py-4 flex items-center gap-3 cursor-pointer hover:border-ios-blue/50 transition">
          {file ? <FileIconForFile file={file}/> : <Paperclip size={16} className="opacity-70"/>}
          <div className="text-sm flex-1 min-w-0 clip-1">
            {file ? file.name : 'PDF, Image, DOC, XLS, PPT, Video… (max 50 MB)'}
          </div>
          {file && (
            <button type="button" onClick={e => { e.preventDefault(); setFile(null); }}
              className="chip text-ios-red shrink-0"><X size={12}/></button>
          )}
          <input type="file"
            ...
            onChange={e => {
              ...
              if (!f) return;
              if (f.size > 50 * 1024 * 1024) { setError('File is larger than 50 MB.'); return; }
              setError(null);
              setFile(f);
              e.target.value = '';
            }}/>
...
function FileIconForFile({ file }: { file: File }) {
  ...
  if (mime.startsWith('image/'))                               return <Image size={16} className="text-ios-green shrink-0"/>;
