...
function ComposeSheet({ targetSemester, onClose }: { targetSemester: number; onClose: () => void }) {
  ...
  const isTeacher = user?.role === 'teacher';
  const myCodes = (myTeacher?.assigned_subjects || []) as string[];
  const semSubjects = useMemo(
    () => allSubjects.filter(s =>
      s.semester === targetSemester && (!isTeacher || myCodes.includes(s.code))
    ),
    [allSubjects, targetSemester, myCodes.join(','), isTeacher]
  );
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      >
        ...
        <select value={subjectId} onChange={e => setSubjectId(e.target.value)}
          ...
          {semSubjects.length === 0 && <option value="">(No assigned subjects for this semester)</option>}
