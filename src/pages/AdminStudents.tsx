...
export default function AdminStudents() {
  ...
  // --- Filter state: Course → Semester → Section ---
  const departments = college?.departments ?? [];
  // Collect all courses across all departments
  const allCourses = useMemo(
    () => departments.flatMap(d => d.courses.map(c => ({ ...c, deptId: d.id }))),
    [departments]
  );
  ...
  // Compute semester list from chosen course
  const chosenCourse = allCourses.find(c => c.id === courseId);
  const semList = chosenCourse?.semesters ?? [];
  ...
  const handleSemesterChange = (s: number | 'All') => {
    setSemester(s);
    setSection('All');
  };

  // Sections from chosen semester
  const sectionList: string[] = useMemo(() => {
    if (semester === 'All' || !chosenCourse) return [];
    return chosenCourse.semesters.find(s => s.number === semester)?.sections ?? [];
  }, [chosenCourse, semester]);

  // Build query filters — only pass defined values to DB query
  const dbFilters = {
  ...
  const { data: rows = [], isLoading, isError, error, refetch } = useCollegeStudents(
    collegeId,
    dbFilters
  );

  // Text search is local (fast) after DB gives us the scoped set
  ...
  // Label for the header count
  const scopeLabel = useMemo(() => {
    const parts: string[] = [];
    if (courseId !== 'All' && chosenCourse) parts.push(chosenCourse.code);
    if (semester !== 'All') parts.push(`Sem ${semester}`);
    if (section  !== 'All') parts.push(`Sec ${section}`);
    return parts.length ? parts.join(' · ') : 'All Courses';
  }, [courseId, semester, section, chosenCourse]);
  ...
  return (
    <div className="space-y-4 min-w-0">
      <div className="card">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex-1 min-w-[160px] no-x">
            ...
            <div className="h-title clip-1">
              {college?.short ?? 'Your college'} · {scopeLabel} · {filtered.length} students
            </div>
          </div>
        ...
        {/* === Filter bar: Course → Semester → Section → Search === */}
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {/* Semester filter — only shown if a course is selected */}
          {courseId !== 'All' && (
            <select
              value={semester === 'All' ? 'All' : String(semester)}
              onChange={e => handleSemesterChange(e.target.value === 'All' ? 'All' : Number(e.target.value))}
              className="px-3 py-2 rounded-xl bg-white/70 dark:bg-white/5 border border-white/60 dark:border-white/10 text-sm"
            >
              <option value="All">All Semesters</option>
              {semList.map(s => (
                <option key={s.id} value={String(s.number)}>Sem {s.label}</option>
              ))}
            </select>
          )}

          {/* Section filter — only shown if a semester is selected */}
          {semester !== 'All' && sectionList.length > 0 && (
            <select
              value={section}
        ...
        {!HAS_SUPABASE && (
          <div className="mt-3 rounded-2xl border border-ios-orange/30 bg-ios-orange/10 px-3 py-2 text-[12px] flex items-start gap-2 text-ios-orange">
            <AlertCircle size={14} className="mt-0.5 shrink-0"/> Supabase is not connected. Add VITE_SUPABASE_URL &amp; VITE_SUPABASE_ANON_KEY to enable.
          </div>
        )}
      </div>
            ...
            <motion.button key={s.id} onClick={() => setOpenStudent(s)}
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] opacity-50 clip-1">{s.course} · Sem {s.semester_number} · Sec {s.section || '—'}</div>
                </div>
              </div>
            </motion.button>
          ))}
