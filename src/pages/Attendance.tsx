...
import {
  ...
  Users, ClipboardList, ArrowLeft, CheckCircle2, Loader2, AlertCircle, Save, BookOpen, Lock
} from 'lucide-react';
...
import {
  ...
  useMyTeacher, useStudentSubjectHistory
} from '../lib/liveData';
...
export default function Attendance() {
  ...
  // Which subjects this user is allowed to take attendance for.
  const { data: subjectList = [] } = useSubjects(user?.college_id);
  const { data: myTeacher } = useMyTeacher(user?.college_id, user?.id);
  const isTeacher = user?.role === 'teacher';
  const teacherSubjectCodes = (myTeacher?.assigned_subjects || []) as string[];

  // If the teacher lands on a semester where they have no assigned
  // subject, silently switch scope to a semester where they DO have one.
  // (Prevents the "No subjects assigned to you for Sem 6" dead-end when
  // the persisted scope from a previous session was Sem 6 but they teach
  // Sem 5.)
  useEffect(() => {
    if (!isTeacher || teacherSubjectCodes.length === 0 || subjectList.length === 0) return;
    const hasHere = subjectList.some(s => s.semester === scope.semester && teacherSubjectCodes.includes(s.code));
    if (hasHere) return;
    const first = subjectList.find(s => teacherSubjectCodes.includes(s.code));
    if (first && first.semester !== scope.semester) {
      scope.setSemester(first.semester);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isTeacher, teacherSubjectCodes.join(','), subjectList.length, scope.semester]);

  const semSubjects = subjectList.filter(s => s.semester === scope.semester);
  const allowedSubjects = useMemo(() => {
    if (!isTeacher) return semSubjects;
    return semSubjects.filter(s => teacherSubjectCodes.includes(s.code));
  }, [isTeacher, semSubjects, teacherSubjectCodes.join(',')]);

  const [subjectId, setSubjectId] = useState('');
  useEffect(() => {
    if (allowedSubjects.length && !allowedSubjects.find(s => s.id === subjectId))
      setSubjectId(allowedSubjects[0].id);
  }, [allowedSubjects.length, scope.semester]);

  const currentSubject = allowedSubjects.find(s => s.id === subjectId);
  ...
  const finalize = async () => {
    ...
    try {
      ...
      setSubmitOk(`Saved ${res.inserted} rows for ${currentSubject?.code || 'this subject'}.`);
  ...
  if (isTeacher && allowedSubjects.length === 0) {
    // Was scope on the wrong semester? Show a switch button.
    const otherSemesterSubject = subjectList.find(s => teacherSubjectCodes.includes(s.code));
    return (
      <div className="card border-ios-orange/30 bg-ios-orange/10">
        <div className="h-title mb-2 flex items-center gap-2 text-ios-orange">
          <Lock size={16}/> No subjects for you in Sem {scope.semester}
        </div>
        <p className="text-sm opacity-80">
          {teacherSubjectCodes.length === 0
            ? 'Your admin has not assigned any subjects to you yet. Ask them to update your Assigned Subjects.'
            : `You are assigned: ${teacherSubjectCodes.join(', ')} — please switch semester to view your class.`}
        </p>
        {otherSemesterSubject && (
          <button onClick={() => scope.setSemester(otherSemesterSubject.semester)}
            className="btn-primary mt-3">
            Go to Sem {otherSemesterSubject.semester}
          </button>
        )}
      </div>
    );
      ...
      <div className="space-y-4">
        <div className="card">
          ...
          <div className="h-title mt-1">
            {currentSubject?.code} · {currentSubject?.name} · Section {scope.section} · {new Date(date).toLocaleDateString()}
          </div>
    ...
    <div className="space-y-4 min-w-0">
      ...
      <section className="card !p-4 relative overflow-hidden"
        ...
        <div className="relative flex items-center gap-3 text-white">
          ...
          {isTeacher && (
            <span className="chip !text-white bg-white/15 border-white/30">
              <Lock size={11}/> Your subject
            </span>
          )}
        </div>
      </section>
      ...
      <section className="card">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
          ...
          <Field label={isTeacher ? 'Subject (yours)' : 'Subject'}>
            {isTeacher && allowedSubjects.length === 1 ? (
              // Only one assigned subject → show as locked chip (no accidental change)
              <div className="input flex items-center gap-1.5 !py-2.5">
                <Lock size={12} className="opacity-60"/>
                <span className="truncate">{allowedSubjects[0].code} · {allowedSubjects[0].name}</span>
              </div>
            ) : (
              <select value={subjectId} onChange={e => setSubjectId(e.target.value)} className="w-full input">
                {allowedSubjects.map(s => <option key={s.id} value={s.id}>{s.code} · {s.name.slice(0,20)}</option>)}
              </select>
            )}
          </Field>
          <Field label="Date">
