...
function toneFor(code: string | null) {
  if (!code) return 'from-black/20 to-black/10';
                  ...
                  return (
                    <div key={ts} className="grid grid-cols-[96px_repeat(6,minmax(150px,1fr))] gap-2 px-3 sm:px-4 py-2 hairline items-stretch">
                      ...
                      {days.map(dow => {
                        ...
                        const tone = toneFor(cell.subject_code);
                        ...
                        return (
                          <div key={dow}
                            ...
                            {cell.room && <div className="text-[10px] opacity-80">Room {cell.room}</div>}
                          </div>
                        );
                      })}
                    </div>
                      ...
                      {dayList.map(s => (
                        <div key={s.id} className="rounded-2xl p-3 border border-white/60 dark:border-white/10 bg-white/60 dark:bg-white/5">
                          ...
                          <div className="text-[11px] opacity-70 mt-1">
                            {s.teacher_name || 'Faculty'}{s.room ? ` · Room ${s.room}` : ''}
                          </div>
                        </div>
                      ))}
                    </div>
