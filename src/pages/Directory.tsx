...
export default function Directory() {
  ...
  const courses    = useMemo(() => Array.from(new Set(all.map(s => s.course).filter(Boolean))).sort(), [all]);
  ...
  // Overall CGPA ranks — computed per (course + semester) group so
  // students are ranked fairly against their actual academic peers.
  const overallRank = useMemo(() => {
    const byCourseSem = new Map<string, typeof all>();
    for (const s of all) {
      const key = `${s.course}:${s.semester_number}`;
      if (!byCourseSem.has(key)) byCourseSem.set(key, []);
      byCourseSem.get(key)!.push(s);
    }
    const map = new Map<string, number>();
    for (const group of byCourseSem.values()) {
      const sorted = [...group].sort((a, b) => (b.cgpa - a.cgpa));
  ...
  const rows = useMemo(() => {
    let arr = all;
    if (course !== 'All')   arr = arr.filter(s => s.course === course);
    ...
    if (q.trim()) {
      ...
      arr = arr.filter(s =>
        ...
        s.course_code?.toLowerCase().includes(t) ||
        s.department?.toLowerCase().includes(t)
      );
    }
    return arr.sort((a, b) => a.sl - b.sl);
  }, [q, course, section, semester, dept, all]);
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] opacity-60 clip-1 mt-0.5" title={`${s.course}${s.department ? ` · ${s.department}` : ''}`}>{s.course} · Sem {s.semester}</div>
              </div>
