...
import {
  ArrowLeft, GraduationCap, Trophy, ClipboardCheck, Layers, Loader2
} from 'lucide-react';
import { useMemo } from 'react';
import { useAuth } from '../lib/auth';
import { useStudentAcademics, useStudentAttendance, useCollegeStudentsWithAttendance } from '../lib/liveData';
...
export default function StudentProfile() {
  ...
  // Load the whole college roster (already cached from Leaderboard/Directory).
  // v_students_public has open SELECT grants so this works for all roles.
  const { data: allStudents = [], isLoading } = useCollegeStudentsWithAttendance(user?.college_id);

  // Find the target student from the already-loaded roster — no extra DB query needed,
  // and no RLS issue since v_students_public bypasses the students table RLS.
  const student = useMemo(
    () => allStudents.find(s => s.reg_no?.toUpperCase() === regNo.toUpperCase()) ?? null,
    [allStudents, regNo]
  );
