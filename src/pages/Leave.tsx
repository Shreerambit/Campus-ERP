...
function StudentView() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const leaveTitle = subj || `${type} leave`;
      await mutateAsync({
      ...
      await dispatchErpNotification({
        ...
        type: 'leave',
        title: `📝 New Leave: ${s.name}`,
        message: `${s.name} (Sem ${s.semester} Sec ${s.section}) applied for ${type} leave: "${leaveTitle}".`,
        link: '/leave',
        entity_id: s.db_id,
        entity_type: 'leaves'
      });

      setMsg({ ok: 'Application submitted and teacher notified!' });
              ...
              {pending.map(h => (
                <ApprovalCard key={h.id} h={h}
                  onDecide={async (status, note) => {
                    await mutateAsync({ id: h.id, status, note });
                    await dispatchErpNotification({
                      college_id: user!.college_id!,
                      ...
                      message: `Your leave request "${h.subject}" was ${status}.${note ? ` Note: ${note}` : ''}`,
                      link: '/leave',
                      entity_id: h.id,
                      entity_type: 'leaves'
                    });
