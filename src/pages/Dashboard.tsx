...
import {
  ...
  AlertTriangle, Target, Lightbulb, Camera as CameraIcon, Building2, Layers
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  if (user && user.role !== 'student' && user.role !== 'parent') {
    return <StaffWelcome name={user.displayName} role={user.role} notices={notices as any} collegeId={user?.college_id} />;
  }
...
function StudentDashboard({ s, allStudents, notices }: { s: Student; allStudents: Student[]; notices: any[] }) {
  ...
  // Pass courseId + semester so rankings are scoped to the same course+semester only.
  const { data: ranks } = useStudentRanks(s.college_id, s.reg_no, s.section, s.course_id, s.semester_number);
...
function StaffWelcome({ name, role, notices, collegeId }: { name: string; role: string; notices: any[]; collegeId?: string }) {
  const nav = useNavigate();
  const { findCollege } = useTenant();
  const college = collegeId ? findCollege(collegeId) : undefined;
  const { data: stats } = useAdminDashboardStats(role === 'admin' ? collegeId : undefined);
    ...
    <div className="space-y-6">
      ...
      <div className="card relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="h-section flex items-center gap-1.5 text-ios-blue font-bold uppercase tracking-wider">
              {role === 'admin' ? (
                <>
                  <Building2 size={14} /> {college?.name ?? 'College Admin Dashboard'}
                </>
              ) : (
                <>{role.toUpperCase()} DASHBOARD</>
              )}
            </div>
            <div className="h-title text-xl sm:text-2xl mt-1">Welcome, {name || 'User'}</div>
            <p className="text-sm opacity-70 mt-1">
              {role === 'teacher' && "Manage today's classes, mark attendance, approve leave, post notices."}
              {role === 'admin' && `${college?.short ?? 'Your college'} · Department & Course administration, students, faculty, and academic operations.`}
            </p>
          </div>

          {role === 'admin' && college && (
            <div className="flex items-center gap-2 self-start sm:self-center bg-white/70 dark:bg-white/5 border border-white/60 dark:border-white/10 px-3.5 py-2 rounded-2xl shrink-0">
              <div className={`h-8 w-8 rounded-xl grid place-items-center text-white font-bold text-sm bg-gradient-to-br ${college.gradient}`}>
                {college.logoText}
              </div>
              <div>
                <div className="font-semibold text-xs">{college.code}</div>
                <div className="text-[10px] opacity-60">{college.city}</div>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Admin College Statistics */}
      {role === 'admin' && stats && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="rounded-3xl p-4 text-white bg-gradient-to-br from-ios-blue to-ios-indigo shadow-hi">
              <div className="text-[11px] opacity-90 flex items-center gap-1 font-semibold uppercase tracking-wider">
                <Users size={13} /> Students
              </div>
              <div className="stat-num mt-1">{stats.studentCount}</div>
            </div>

            <div className="rounded-3xl p-4 text-white bg-gradient-to-br from-ios-purple to-ios-pink shadow-hi">
              <div className="text-[11px] opacity-90 flex items-center gap-1 font-semibold uppercase tracking-wider">
                <GraduationCap size={13} /> Faculty
              </div>
              <div className="stat-num mt-1">{stats.teacherCount}</div>
            </div>

            <div className="rounded-3xl p-4 text-white bg-gradient-to-br from-ios-green to-ios-teal shadow-hi">
              <div className="text-[11px] opacity-90 flex items-center gap-1 font-semibold uppercase tracking-wider">
                <Layers size={13} /> Courses
              </div>
              <div className="stat-num mt-1">{stats.courseCount}</div>
            </div>

            <div className="rounded-3xl p-4 text-white bg-gradient-to-br from-ios-orange to-ios-red shadow-hi">
              <div className="text-[11px] opacity-90 flex items-center gap-1 font-semibold uppercase tracking-wider">
                <Building2 size={13} /> Departments
              </div>
              <div className="stat-num mt-1">{stats.departmentCount}</div>
            </div>

            <div className="rounded-3xl p-4 text-white bg-gradient-to-br from-ios-pink to-rose-500 shadow-hi">
              <div className="text-[11px] opacity-90 flex items-center gap-1 font-semibold uppercase tracking-wider">
                <Megaphone size={13} /> Notices
              </div>
              <div className="stat-num mt-1">{stats.noticeCount}</div>
            </div>

            <div className="rounded-3xl p-4 text-white bg-gradient-to-br from-amber-500 to-ios-orange shadow-hi">
              <div className="text-[11px] opacity-90 flex items-center gap-1 font-semibold uppercase tracking-wider">
                <FileSignature size={13} /> Leaves
              </div>
              <div className="stat-num mt-1">{stats.pendingLeaves} pending</div>
            </div>
          </div>

          {/* Course & Semester Distribution */}
          {stats.courseBreakdown.length > 0 && (
            <div className="card">
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="h-title text-sm font-semibold">Academic Distribution (Course &amp; Semester)</div>
                <button onClick={() => nav('/admin/students')} className="chip text-xs">Manage Students →</button>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                {stats.courseBreakdown.map(b => (
                  <div key={`${b.courseId}-${b.semester}`}
                    className="p-3 rounded-2xl bg-white/70 dark:bg-white/5 border border-white/60 dark:border-white/10 text-center">
                    <div className="font-bold text-sm text-ios-blue">{b.courseCode}</div>
                    <div className="text-xs opacity-60">Sem {b.semester}</div>
                    <div className="font-extrabold text-base mt-1">{b.count} <span className="text-[10px] font-normal opacity-60">students</span></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {role === 'teacher' && <>
          ...
          <Quick icon={<Trophy size={18} />} title="Ranks" desc="Student leaderboard" to="/leaderboard" />
          <Quick icon={<BookOpen size={18} />} title="Notes" desc="Upload study materials" to="/notes" />
          <Quick icon={<CalendarClock size={18} />} title="Timetable" desc="Weekly schedule" to="/timetable" />
          ...
          <Quick icon={<Users size={18} />} title="Directory" desc="Student roster" to="/directory" />
          <Quick icon={<GraduationCap size={18} />} title="Academics" desc="Marks & results" to="/academics" />
        </>}
        {role === 'admin' && <>
          <Quick icon={<Users size={18} />} title="Students" desc="Add · edit · filter" to="/admin/students" />
          <Quick icon={<GraduationCap size={18} />} title="Teachers" desc="Usernames · reset PW" to="/admin/teachers" />
          <Quick icon={<Trophy size={18} />} title="Ranks" desc="College leaderboard" to="/leaderboard" />
          <Quick icon={<CalendarClock size={18} />} title="Timetable" desc="Upload / edit" to="/admin/timetable" />
          <Quick icon={<Megaphone size={18} />} title="Notices" desc="Post college notices" to="/notices" />
          <Quick icon={<FileSignature size={18} />} title="Leave Requests" desc="Review applications" to="/leave" />
          <Quick icon={<Upload size={18} />} title="Import" desc="CSV / XLSX bulk upload" to="/import" />
          <Quick icon={<Users size={18} />} title="Directory" desc="Browse all students" to="/directory" />
        </>}
      </div>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {notices.slice(0, 5).map((n: any) => (
              <div key={n.id} className="flex items-start gap-3 rounded-2xl p-3 bg-white/60 dark:bg-white/5 border border-white/60 dark:border-white/10">
                <Megaphone size={16} className="mt-1 text-ios-pink shrink-0" />
                <div className="min-w-0 flex-1">
                  <div className="font-semibold text-sm clip-1">{n.title}</div>
