...
export default function Leaderboard() {
  ...
  const studentCourse = user?.student?.course;
  const [sort, setSort] = useState<Sort>('cgpa');
  const [course, setCourse] = useState<string>(() => (isStudent && studentCourse ? studentCourse : 'All'));
  ...
  const queryFilters = useMemo(() => {
    if (isStudent && user?.student?.course_id) {
      return { courseId: user.student.course_id };
    }
    return {};
  }, [isStudent, user?.student?.course_id]);

  const { data: all = [], isLoading, isError, error, refetch, isFetching } = useCollegeStudentsWithAttendance(collegeId, queryFilters);
  ...
  const rows = useMemo(() => {
    ...
    return arr;
  }, [sort, course, section, semester, q, all]);
