...
import { NotificationBell, NotificationToast } from './NotificationCenter';
import ErpAiChatWidget from './ErpAiChatWidget';
...
export default function Shell() {
  ...
  // Role-specific bottom nav: exactly 5 tabs, curated per role
  const STUDENT_BOTTOM  = ['/dashboard', '/timetable', '/notes', '/notices', '/leaderboard'];
  const TEACHER_BOTTOM  = ['/dashboard', '/attendance', '/leaderboard', '/notes', '/timetable'];
  const DEFAULT_BOTTOM  = ['/dashboard', '/timetable', '/notes', '/notices', '/leaderboard'];

  const bottomPaths =
    user?.role === 'student' ? STUDENT_BOTTOM :
    user?.role === 'teacher' ? TEACHER_BOTTOM :
    DEFAULT_BOTTOM;

  const bottomItems = bottomPaths
    .map(path => items.find(n => n.to === path))
  ...
  return (
    <div className="min-h-dvh flex">
      ...
      <aside className="hidden md:flex w-[240px] lg:w-[260px] shrink-0 flex-col gap-2 p-4 lg:p-5 sticky top-0 h-dvh">
        ...
        <div className="mt-auto card !p-3">
          <div className="flex items-center gap-2 min-w-0">
            {user?.photo && <img src={user.photo} className="h-9 w-9 rounded-xl border border-white/60 bg-white shrink-0 object-cover"/>}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="hidden md:flex items-center justify-between px-8 py-3.5 border-b border-black/5 dark:border-white/5 sticky top-0 z-30 bg-white/40 dark:bg-black/20 backdrop-blur-md">
          <div className="flex items-center gap-2.5">
            <h1 className="text-base font-bold text-gray-900 dark:text-white capitalize">
              {loc.pathname === '/' || loc.pathname === '/dashboard' ? 'Dashboard' : loc.pathname.replace('/', '').replace('admin/', '').replace('-', ' ')}
            </h1>
            <span className="text-xs opacity-50">· {college?.name || 'Basaveshwar Science College'}</span>
          </div>
          <div className="flex items-center gap-2.5">
            <NotificationBell />
            <button onClick={toggle} className="h-9 w-9 rounded-full glass grid place-items-center hover:scale-105 transition" aria-label="Toggle theme">
              {theme === 'dark' ? <Sun size={16}/> : <Moon size={16}/>}
            </button>
            <button
              onClick={() => nav('/profile')}
              className="flex items-center gap-2 glass rounded-full pl-1.5 pr-3 py-1 ring-1 ring-white/30 hover:scale-105 transition"
            >
              {user?.photo ? (
                <img src={user.photo} className="h-7 w-7 rounded-full object-cover" />
              ) : (
                <div className="h-7 w-7 rounded-full bg-ios-blue text-white grid place-items-center text-xs font-bold">
                  {user?.displayName?.[0] || 'U'}
                </div>
              )}
              <span className="text-xs font-semibold clip-1 max-w-[120px]">{user?.displayName || 'Profile'}</span>
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 min-w-0 px-3 sm:px-4 md:px-8 pt-4 md:pt-6" style={{ paddingBottom: 'calc(6.5rem + env(safe-area-inset-bottom))' }}>
      ...
      {/* Floating ERP AI Chatbot Assistant */}
      <ErpAiChatWidget />
    </div>
  );
}
