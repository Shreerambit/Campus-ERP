...
import {
  ...
  User, LogOut, Sun, Moon, Users, Upload, Megaphone
} from 'lucide-react';
      ...
      <aside className="hidden md:flex w-[240px] lg:w-[260px] shrink-0 flex-col gap-2 p-4 lg:p-5 sticky top-0 h-dvh">
        ...
        <div className="mt-auto card !p-3">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            ...
            <button onClick={() => { logout(); nav('/welcome'); }} className="chip justify-center !text-ios-red">
        ...
        <div className="md:hidden sticky top-0 z-40">
          <div className="mx-3 mt-3 rounded-3xl glass px-3 py-2.5 flex items-center justify-between gap-2 min-w-0">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button onClick={() => { logout(); nav('/welcome'); }} className="h-9 w-9 rounded-full glass grid place-items-center text-ios-red" aria-label="Log out">
                <LogOut size={16}/>
              </button>
            </div>
          </div>
...
/* ---------- Brand: real Campus-ERP logo + secondary college line ---------- */
  ...
  return (
    <div className="flex items-center gap-2.5 min-w-0 flex-1">
      <img
        ...
        alt="Campus ERP"
        className={`shrink-0 rounded-[22%] shadow-hi ring-1 ring-white/10 ${small ? 'h-9 w-9' : 'h-11 w-11'}`}
      />
      <div className="min-w-0 flex-1 leading-tight">
        <div className={`font-bold tracking-tight bg-clip-text text-transparent block clip-1
          ${small ? 'text-[14px]' : 'text-[16px]'}`}
          style={{ backgroundImage: 'linear-gradient(135deg,#307DFF 0%,#3C3DFF 55%,#7F23FF 100%)' }}>
          Campus&nbsp;ERP
        </div>
        <div className="text-[10px] sm:text-[11px] opacity-70 clip-1 mt-0.5">
          {displayCollege}
        </div>
      </div>
    </div>
...
export function GradCapIcon({ size = 24 }: { size?: number }) {
  return <img src="/brand-icon.png?v=3" alt="" width={size} height={size} className="rounded-[22%]" style={{ display: 'block' }} />;
}
